...
def normalize_bbox_psc(bboxes, pc_range):
    cx = bboxes[..., 0:1]
    cy = bboxes[..., 1:2]
    cz = bboxes[..., 2:3]
    w = bboxes[..., 3:4].log()
...
def denormalize_bbox_psc(normalized_bboxes, pc_range):
    # rotation 
    ...
    phase_sin = torch.sum(
        rot_enc[:, 0: 3] * coef_sin,
        dim=-1,
        keepdim=False)
    phase_cos = torch.sum(
        rot_enc[:, 0: 3] * coef_cos,
        dim=-1,
        keepdim=False)
    ...
    phase[phase_mod < thr_mod] *= 0
    rot = phase.unsqueeze(-1)
    ...
    if normalized_bboxes.size(-1) > 8:
         # velocity 
        vx = normalized_bboxes[:, 9:10]
        vy = normalized_bboxes[:, 10:11]
        denormalized_bboxes = torch.cat([cx, cy, cz, w, l, h, rot, vx, vy], dim=-1)
    else:
        denormalized_bboxes = torch.cat([cx, cy, cz, w, l, h, rot], dim=-1)
    return denormalized_bboxes
