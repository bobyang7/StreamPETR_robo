...
class FocalAngleHead_V1(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        super(FocalAngleHead_V1, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def prepare_for_dn(self, batch_size, reference_points, img_metas):
        if self.training and self.with_dn:
            targets = [torch.cat((img_meta[0]['gt_bboxes_3d']._data.gravity_center, img_meta[0]['gt_bboxes_3d']._data.tensor[:, 3:]),dim=1) for img_meta in img_metas ]
            labels = [img_meta[0]['gt_labels_3d']._data for img_meta in img_metas ]
    ...
    def forward(self, img_metas, outs_roi=None, **data):
        ...
        # if self.add_query_from_2d:
            ...
            if self.return_context_feat:        # 2D 目标中心所在位置的特征
            ...
            if flag_use_gt_depth:
                ...
                pred_depth_ = [img_meta[0]['depths'].data[1] for img_meta in img_metas]
            ...
            padHW = img_metas[0][0]['pad_shape'][0][:2]
            if self.use_offline_2d:
                pred_bbox_list = img_metas[0][0]['offline_2d']     # currently only for bs=1, BN*(M, 6), float64
    ...
    def get_targets(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list,
         bbox_weights_list, pos_inds_list, neg_inds_list) = multi_apply(
             ...
             [i[0] for i in gt_labels_list], gt_bboxes_list, gt_bboxes_ignore_list)
    ...
    def loss(self,
        ...
        device = gt_labels_list[0][0].device
        gt_bboxes_list = [torch.cat(
            (gt_bboxes[0].gravity_center, gt_bboxes[0].tensor[:, 3:]),
