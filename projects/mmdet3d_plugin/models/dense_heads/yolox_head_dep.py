...
class YOLOXHeadCustomDepth(BaseDenseHead, BBoxTestMixin):
    ...
    def loss(self,
        ...
        if self.pred_with_depth:
            ...
            if 'depths' in img_metas[0][0].keys():
                # load gt depths in pipeline
                gt_ins_depth = [img_meta[0]['depths'].data for img_meta in img_metas]
                ...
                if len(gt_ins_depth) == 1:
                    ...
                    gt_depths = []
                    for i in range(len(gt_ins_depth)):
                        gt_depths += gt_ins_depth[i]
                    # gt_depths = torch.cat(gt_ins_depth).to(device)
            else:
                gt_depths = None
            ...
            if self.multi_level_pred:   # TODO deprecated
                ...
                if self.depthnet_config['type'] in [0, 2]:
                    ...
                    loss_depth_map = depth_map_logits.sum() * 0
                    loss_dict.update(loss_depth=loss_depth_map)
                if self.depthnet_config['type'] in [1, 2]:
