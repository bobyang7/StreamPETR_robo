...
class FocalAngleHead_V2(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if 'rec_norm' in kwargs:
            self.rec_norm = kwargs['rec_norm']
        else:
            self.rec_norm = False
        ...
        if class_weight is not None and (self.__class__ is FocalAngleHead_V2):
        ...
        super(FocalAngleHead_V2, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _init_layers(self):
        ...
        if (self.return_context_feat or self.return_bbox2d_scores) and self.add_query_from_2d:
...
#bd
    ...
    def build_query2d_proposal(self, pred_bbox_list, pred_depth, data, bn, padHW,
                               ...
                               context2d_feat=None, bbox2d_scores=None, flag_use_gt_depth=False):
        '''
        ...
        img2lidars = torch.inverse(data['lidar2img'].cpu()).to('cuda')  # (B, N, 4, 4)
        ...
        context2d_feat = context2d_feat.view(B, -1, context2d_feat.shape[-1]) if context2d_feat is not None else None
        new_reference_points = coords3d.view(B, -1, coords3d.shape[-1])
    ...
    def forward(self, img_metas, outs_roi, **data):
        ...
        feat_flatten_2d = torch.cat([feat_flatten[1]], dim=1)
        ...
        if self.add_query_from_2d:
            ...
            # 2D 目标中心所在位置的特征
            _dim = feat_flatten_2d.shape[-1]
            valid_indices = outs_roi['valid_indices']
            context_feat = feat_flatten_2d[valid_indices.repeat(1, 1, _dim)].reshape(-1, _dim)

            context2d_feat = context_feat.detach()

            flag_use_gt_depth = (self.training and self.train_use_gt_depth)
            if flag_use_gt_depth:
                # train use gt depth for regression; val process use gt depth for exploring upper bound
                # gt_ins_depth = [img_meta[0]['depths'] for img_meta in img_metas]
                device = pred_depth.device
                pred_depth_ = [img_meta['depths'].data[1] for img_meta in img_metas]

                pred_depth_ = torch.stack(pred_depth_, dim=0).flatten(0, 1).unsqueeze(-1).to(device)
                pred_depth_ = torch.clamp(pred_depth_, min=self.depthnet_config['grid_config']['depth'][0], max=self.depthnet_config['grid_config']['depth'][1])
            ...
            reference_points2d, context_feat = self.build_query2d_proposal(pred_bbox_list, depth_input, data, (B, N),
                ...
                context2d_feat=context2d_feat, bbox2d_scores=bbox2d_scores, flag_use_gt_depth=flag_use_gt_depth)
    ...
    def _convert_bin_depth_to_specific(self, pred_indices, mode='LID', inverse=False):
        depth_min, depth_inval, num_bins = self.depthnet_config['grid_config']['depth'][0], self.depthnet_config['grid_config']['depth'][2], self.depthnet_config['depth_channels']
        
        return (pred_indices * depth_inval) + (depth_min - depth_inval)
    ...
    def loss_single(self,
        ...
        if self.rec_norm:
            normalized_bbox_targets = normalize_bbox_rec(bbox_targets, self.pc_range)
        else:
