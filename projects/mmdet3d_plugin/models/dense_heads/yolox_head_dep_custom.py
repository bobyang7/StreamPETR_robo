...
class YOLOXHeadCustomDepth_CUSTOM(BaseDenseHead, BBoxTestMixin):
    ...
    def init_weights(self):
        super(YOLOXHeadCustomDepth_CUSTOM, self).init_weights()
    ...
    def get_bboxes(self, preds_dicts,
        ...
        if False:   # 不用全部尺度的，太浪费时间
            ...
            cls_scores = [preds_dicts['enc_cls_scores'][1]]      # shape 3x(BN num_cls Hi Wi), they are logits
            bbox_preds = [preds_dicts['enc_bbox_preds'][1]]      # shape 3x(BN 4 Hi Wi)
            objectnesses = [preds_dicts['objectnesses'][1]]      # shape 3x(BN 1 Hi Wi)          
        ...
        single_priors = self.prior_generator.single_level_grid_priors(featmap_sizes[0], 1, dtype=cls_scores[0].dtype, device=cls_scores[0].device, with_stride=True)
        
        mlvl_priors = [single_priors]
    ...
    def get_depth_loss(self, depth_preds, depth_labels):
        ...
        multi_level = len(depth_labels[0])
        bs = len(depth_labels)
        gt_depths = []
        ...
        for level in range(multi_level):
            gt_depths_ = []
            for b in range(bs):
                gt_depths_.append(depth_labels[b][level])
