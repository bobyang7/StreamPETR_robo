...
class Petr3D(MVXTwoStageDetector):
    ...
    def extract_img_feat(self, img, len_queue=1, training_mode=False):
        ...
        if img is not None:
            ...
            #输入[12,3,256,704]，输出[12, 1024, 16, 44]和[12, 2048, 8, 22], 其中12是B*N
        ...
        if self.with_img_neck:
            ...
            #输入[12, 1024, 16, 44]和[12, 2048, 8, 22]，输出[12, 256, 16, 44]和[12, 256, 8, 22]
        ...
        return img_feats_reshaped  # (bs, T', N, C, H, W) 例如 [2,1,6,256,16,44]
    ...
    def prepare_location(self, img_metas, **data):
        ...
        # 对[16,44]的特征图生成每个网格点的中心坐标，归一化在0-1之间
    ...
    def forward_pts_train(self,
        ...
        if not requires_grad:
            ...
            topk_indexes = outs_roi['topk_indexes'] #每个token是正例的置信度的topk索引
    ...
    def simple_test_pts(self, img_metas, **data):
        ...
        velo_pre = outs['all_bbox_preds'][..., 8:10]
        # velo_pre = outs['all_bbox_preds'][..., 8:10].clone()

        # log编码方案：
        # mask_pos = velo_pre >= 0
        # mask_neg = velo_pre < 0
        # # bbox_preds_velo = (torch.exp(torch.tensor(1)) ** velo_pre - 1) * mask_pos + (1 - torch.exp(torch.tensor(1)) ** (-1 * velo_pre)) * mask_neg
        # # bbox_preds_velo = (10 ** velo_pre - 1) * mask_pos + (1 - 10 ** (-1 * velo_pre)) * mask_neg
        # bbox_preds_velo = (2 ** velo_pre - 1) * mask_pos + (1 - 2 ** (-1 * velo_pre)) * mask_neg
        
        # a = bbox_preds_velo.max()
        # b = bbox_preds_velo.min()
        # print(b,a)
        
        # gaussian编码方案：
        bbox_preds_velo = 0 + 15 * torch.sqrt(torch.tensor(2.0)) * torch.erfinv(2 * torch.clamp(velo_pre, min=0.0001, max=0.9999) - 1)

        outs['all_bbox_preds'] = torch.cat((outs['all_bbox_preds'][..., :8], bbox_preds_velo), dim = -1)
        
        #速度分bin
        # velo_cls_vx = outs['all_cls_scores_vx']
        # velo_vx = torch.argmax(velo_cls_vx, axis = -1) - 50 + outs['all_bbox_preds_v'][..., 0]
        # outs['all_bbox_preds'][..., 8] = velo_vx
        # c = velo_vx.max()
        # # d = velo_vx.min()
        # d = velo_pre[...,0].max()
        # print('vx是: ',c,d)

        # velo_cls_vy = outs['all_cls_scores_vy']
        # velo_vy = torch.argmax(velo_cls_vy, axis = -1) - 50 + outs['all_bbox_preds_v'][..., 1]
        # outs['all_bbox_preds'][..., 9] = velo_vy
        # e = velo_vy.max()
        # # f = velo_vy.min()
        # f = velo_pre[...,1].max()
        # print('vy是: ',e,f)
