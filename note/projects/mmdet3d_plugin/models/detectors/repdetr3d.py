...
class RepDetr3D(MVXTwoStageDetector):
    ...
    def forward_pts_train(self,
        ...
        if not requires_grad:
            ...
            outs_roi = self.forward_roi_head(**data) # 返回的是4个尺寸的特征图，每个尺寸的6张分别对应的分类特征图，回归特征图，中心点偏移特征图，正样本特征图
            outs = self.pts_bbox_head(img_metas, **data)
    ...
    def forward(self, return_loss=True, **data):
        ...
        """
        if return_loss:
            # 'gt_bboxes_3d', 'gt_labels_3d'是3d框的gt，
            # 'gt_bboxes', 'gt_labels'是6张环视2d图像的gt框，
            # 'centers2d', 'depths'也是6张环视2d图像的gt框的中心和深度
