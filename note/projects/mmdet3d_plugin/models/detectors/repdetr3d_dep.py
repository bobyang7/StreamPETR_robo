...
class RepDetr3DDepth(MVXTwoStageDetector):
    ...
    def prepare_location(self, img_metas, **data):
        ...
        for i in range(len(data['img_feats'])):
            ...
            # 提取特征图之前的尺度是[800,320],stride则是下采样率，如stride为4时，x是[200,80]
            # pad_h和pad_w就是320和800
            # 返回的location是特征图的所有网格中心位置，位置进行了归一化
            location = locations(x, self.stride[i], pad_h, pad_w)[None].repeat(bs*n, 1, 1, 1)
            location_r.append(location)
        #返回的列表是4中不同尺度特征图的网格的中心位置，如[200,80],[100,40],[50,20],[25,10]四种特征图的网格中心点位置
        return location_r
    ...
    def forward_pts_train(self,
        ...
        # 这里的roi_head是yoloX
        # 输出是所有尺度特征图回归和分类的结果
        # 回归的有框的大小位置，fcos3d中的中心点偏移，和深度
        # 分类的有框的类别和objectness正负样本
        outs_roi = self.forward_roi_head(location, **data)

        # yoloX根据不同尺度特征图的anchor_free峰值点，取出所有的正样本2d框
        ...
        # pts_bbox_head这里进入到repdetr3d(类似StreamPETR)的主体代码部分
