...
class PETRTemporalTransformer(BaseModule):
    ...
    def forward(self, memory, tgt, query_pos, pos_embed, attn_masks, temp_memory=None, temp_pos=None, mask=None, reg_branch=None):
        ...
        Returns:
            tuple[Tensor]: results of decoder containing the following tensor.
                ...
                - memory: Output results from encoder, with shape \
                      [bs, embed_dims, h, w].
        """
        memory = memory.transpose(0, 1).contiguous()    #[2, 4224, 256] -> [4224,2,256]
        query_pos = query_pos.transpose(0, 1).contiguous()  #[2, 758, 256] -> [758,2,256]
        pos_embed = pos_embed.transpose(0, 1).contiguous()  #[2, 4224, 256] -> [4224,2,256]
        ...
        if tgt is None:
            ...
            tgt = tgt.transpose(0, 1).contiguous()  #[2,758,256] -> [758,2,256]

        if temp_memory is not None: #[2,384,256]
            temp_memory = temp_memory.transpose(0, 1).contiguous() #[384,2,256]
            temp_pos =  temp_pos.transpose(0, 1).contiguous()

        # out_dec: [num_layers, num_query, bs, dim]
        out_dec = self.decoder(
            query=tgt,  #[658,2,256]
            key=memory,    #[4224,2,256]
            value=memory,   
            key_pos=pos_embed,  #[4224,2,256]
            query_pos=query_pos,    #[658,2,256]
            temp_memory=temp_memory,    #[384,2,256]
            temp_pos=temp_pos,  #[384,2,256]
            key_padding_mask=mask,
            attn_masks=[attn_masks, None],  #[658,1042]
