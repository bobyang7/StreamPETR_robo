...
class SparseHeadDepth(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        super(SparseHeadDepth, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def prepare_for_dn(self, batch_size, reference_points, img_metas):
        if self.training and self.with_dn:
            ...
            if self.bbox_noise_scale > 0:
                ...
                #正样本占1份，负样本占2份，即正样本有10组45个dn框，负样本有10组90个dn框
                ...
                for i in range(groups):
                    # cdist用来计算两组张量的各元素间的距离
                    # 计算每一组的135个正负dn框和45个gt框的距离，包括bs的距离会被无限放大，以及中心点的距离
                    cost_bs = torch.cdist(batch_idx_pred[i].unsqueeze(-1), batch_idx_gt[i].unsqueeze(-1), p=1)
                    
                    # known_bbox_center拼接了所有正例10*99和负例噪声10*36
                ...
                # WHY 这里为什么要限制在0-1，框的wlh和速度也要限制在0-1吗
            ...
            mask_dict = { #假设一个batch有两个sample，共有33+12个gt
                'known_indice': torch.as_tensor(known_indice).long(), # 则为0-45*3即 0-134，循环10次
                'batch_idx': torch.as_tensor(batch_idx).long(), # 则为33*3个0和12*3个1
                'map_known_indice': torch.as_tensor(map_known_indice).long(), # 上面那个很长的举例注释
                'known_lbs_bboxes': (known_labels, known_bboxs),  # 450个dn框的标签和框，这里竟然没有乘3
                'know_idx': know_idx, # 33个1，和12个1
                'costs':costs,
                'pad_size': pad_size #取决于一个batch中gt最多的那个sample的10*3倍
            }
    ...
    def forward(self, img_metas, outs_roi=None, **data):
        ...
        """
        reference_points:[bs, 990+644, 3] 
        1634 其中的990是(bs中gt最多的33 * 10组dn框 * 3（1:2的正负dn比例)）
        644是 本来的query数量

        attn_mask: [1890,2658]
        1890是 (990 + 644 + propagated为256)
        2658是 (990 + 644 + memory_len为1024-256)

        mask_dict请进入函数内部看返回部分的注释
        """
        ...
        for lvl in range(outs_dec.shape[0]):
            ...
            # reference_points本来是小数级别，通过inverse_sigmoid变到整数级别
            # 下面的回归的“偏移”也是小数级别，加到整数级别的reference_points上再进行sigmoid
            # 由此完成一次位置的更新，注意回归的是"偏移量"
    ...
    def prepare_for_loss(self, mask_dict):
        ...
        if len(output_known_class) > 0:
            # [1350, 6, 10], 1350是bs中第一个样本的990个dn和第二个样本的360个dn，6是6层decoder的分类结果
