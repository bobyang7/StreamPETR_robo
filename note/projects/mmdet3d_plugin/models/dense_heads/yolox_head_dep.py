...
class YOLOXHeadCustomDepth(BaseDenseHead, BBoxTestMixin):
    ...
    def init_weights(self):
        super(YOLOXHeadCustomDepth, self).init_weights()
    ...
    def get_bboxes(self, preds_dicts,
        ...
        # mlvl_priors是多层的特征的先验框，比如[200,80]的特征图的stride是4
        # 对应的先验框就是特征图上的每个网格的点为中心，长宽为stride的所有框
        ...
        # 预测框的解码规则：
        # 预测框中心点xy是：预测框的xy * 先验框的长款 + 先验框的xy
        # 预测框长宽wh是：预测框的wh的exp * 先验框的wh
        ...
        # 上面的flatten_bbox有了所有位置的预测框，
        # valid_indices有了所有位置是否是正样本
        # 这里对B*N张图像的所有正样本取出来
        result_list = []
    ...
    def _get_target_single(self, cls_preds, objectness, priors, decoded_bboxes,
        ...
        Args:
            ...
            priors (Tensor): All priors of one image, a 2D-Tensor with shape
                [num_priors, 4] in [cx, xy, stride_w, stride_y] format. 下采样率越大，先验框越大
            ...
            gt_labels (Tensor): Ground truth labels of one image, a Tensor
                with shape [num_gts]. 一张图像的所有GT label
        """
        ...
        # YOLOX使用的先验框都是特征图网格的角点，而不是网格的中心点，所以需要变为中心点
        offset_priors = torch.cat(
            [priors[:, :2] + priors[:, 2:] * 0.5, priors[:, 2:]], dim=-1)

        # 既有offset_prior先验框，又有decoded_bbox解码框，这两个怎么作用和gt框来匹配？
