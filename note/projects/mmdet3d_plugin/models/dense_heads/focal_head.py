...
class FocalHead(AnchorFreeHead):
    ...
    def forward(self, location, **data):
        ...
        x = src.flatten(0, 1) #[bs*n, 256, 16, 44], 一共有16*44=704个图像token
        cls_feat = self.shared_cls(x)
        cls = self.cls(cls_feat) #[bs*n, 10, 16, 44]
        centerness = self.centerness(cls_feat) #[bs*n, 1, 16, 44]
        cls_logits = cls.permute(0,2,3,1).reshape(bs*n,-1,self.num_classes) #[bs*n, 704, 10]
        centerness = centerness.permute(0,2,3,1).reshape(bs*n,-1,1) #[bs*n, 704, 1]
        pred_bboxes = None
        pred_centers2d = None
        ...
        outs = {
                'enc_cls_scores': cls_logits, #每个图像token的10分类置信度
                'enc_bbox_preds': pred_bboxes, #每个图像token的预测框的大小
                'pred_centers2d': pred_centers2d, #每个图像token的预测中心点的偏移
                'centerness':centerness, #每个图像token对应是正例物体的置信度
                'topk_indexes':topk_indexes, #每个图像token按照置信度排名的索引
            }
